...
<link rel="stylesheet" media="screen,projection" type="text/css" href="templates/css/botton.css" />
	<div id="section" class="box">
		<button class="button black " onclick="javascript:window.location.href='http://{{$smarty.const.global_localhost}}/blog_maizi/blog/?action=postblog';"> 发 帖 </button>
		...
		<ul class="articles box">
			{{if !empty($data.myblog_list)}}
				{{foreach from=$data.myblog_list item=item key=key}}
					<li>
						...
						<p class="more"><a href="/blog_maizi/blog/?action=deleteBlog&id={{$item.id}}&page={{$pageInfo.page}}" 
			...
			<div class="pagination box">
				<p class="f-right">
					{{if $pageInfo.page > 1}}
						<a href="http://{{$smarty.const.global_localhost}}/blog_maizi/blog/?action=myblog&page={{$pageInfo.page-1}}">上一页 &raquo;</a>
					{{/if}}
					{{foreach from=$pageInfo.page_arr item=item}}
						<a href="http://{{$smarty.const.global_localhost}}/blog_maizi/blog/?action=myblog&page={{$item}}" {{if $item==$pageInfo.page}}class="current"{{/if}}>{{$item}}</a>
					{{/foreach}}
					{{if $pageInfo.page < $pageInfo.page_max}}
					<a href="http://{{$smarty.const.global_localhost}}/blog_maizi/blog/?action=myblog&page={{$pageInfo.page+1}}">下一页 &raquo;</a>
					{{/if}}
				</p>
