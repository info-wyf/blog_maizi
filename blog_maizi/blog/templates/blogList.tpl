	...
	<div id="section" class="box">
		...
		<ul class="articles box">
			{{if !empty($data.bloglist)}}
				{{foreach from=$data.bloglist item=item key=key}}
					<li>
						...
						<p class="more"><a href="http://{{$smarty.const.global_localhost}}/blog_maizi/blog/?action=deleteBlog&id={{$item.id}}&page={{$pageInfo.page}}" 
			...
			<div class="pagination box">
				<p class="f-right">
					{{if $pageInfo.page > 1}}
						<a href="http://{{$smarty.const.global_localhost}}/blog_maizi/blog/?action=bloglist&page={{$pageInfo.page-1}}&user_id={{$params.safe.user_id}}">上一页 &raquo;</a>
					{{/if}}
					{{foreach from=$pageInfo.page_arr item=item}}
						<a href="http://{{$smarty.const.global_localhost}}/blog_maizi/blog/?action=bloglist&page={{$item}}&user_id={{$params.safe.user_id}}" {{if $item==$pageInfo.page}}class="current"{{/if}}>{{$item}}</a>
					{{/foreach}}
					{{if $pageInfo.page < $pageInfo.page_max}}
					<a href="http://{{$smarty.const.global_localhost}}/blog_maizi/blog/?action=bloglist&page={{$pageInfo.page+1}}&user_id={{$params.safe.user_id}}">下一页 &raquo;</a>
					{{/if}}
				</p>
